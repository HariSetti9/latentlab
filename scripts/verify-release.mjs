...
if (pendingCount === 0) {
  ...
  if (!videoIsPublic) failures.push('submission/FINAL_LINKS.md: Demo Video must be a real HTTPS URL');
}
if (failures.length || pendingCount > 0) {
  console.error([...failures, ...(pendingCount ? [`${pendingCount} pending release URL markers remain`] : [])].join('\n'));
  process.exit(1);
}
console.log('Strict release URL gate passed: every deliverable URL is complete.');
