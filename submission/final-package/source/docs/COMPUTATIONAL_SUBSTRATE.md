...
This system is `LIVE COMPUTATION` and an `ALGORITHMIC TOY`. It is not BDH, BDH-CQ, a reproduction of either paper, a language model, or evidence about frontier-model cognition. The analogy is only that a fixed-size internal state is updated repeatedly before output is read.
